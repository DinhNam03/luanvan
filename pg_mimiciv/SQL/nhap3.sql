-- DROP TABLE mimiciv_derived.sepsis
-- CREATE TABLE mimiciv_derived.sepsis AS
WITH ap AS (
...
SELECT * FROM mimiciv_derived.sepsis WHERE  
alt IS NOT NULL 
AND ast IS NOT NULL 
...
SELECT subject_id, hadm_id, charttime,specimen_id,
	MAX(CASE WHEN itemid = 51000 THEN valuenum ELSE NULL END) AS triglycerides
FROM mimiciv_hosp.labevents WHERE itemid = 51000 AND had
 GROUP BY subject_id, hadm_id, charttime, specimen_id;
