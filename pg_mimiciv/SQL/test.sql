--DROP TABLE mimiciv_derived.sepsis2
-- CREATE TABLE mimiciv_derived.sepsis2 AS
WITH ap AS (
  -- Bệnh nhân viêm tụy cấp: 7510
	SELECT subject_id, hadm_id FROM mimiciv_hosp.diagnoses_icd WHERE icd_code = '5770' OR icd_code LIKE 'K85%'
),
ap_icu AS (
  -- Thông tin ICU: 74843
...
first_labs AS (
  SELECT ie.stay_id,
         ...
         AVG(hemoglobin) AS hemoglobin, MAX(platelets) AS platelets,
...
SELECT * FROM mimiciv_derived.sepsis2 
WHERE triglycerides IS NOT NULL
AND pt IS NOT NULL
AND ptt IS NOT NULL
AND inr IS NOT NULL;
...
SELECT subject_id, hadm_id,
	MAX(CASE WHEN itemid = 51000 THEN valuenum ELSE NULL END) AS triglycerides
FROM mimiciv_hosp.labevents WHERE itemid = 51000 AND hadm_id IS NOT NULL
 GROUP BY subject_id, hadm_id
...
SELECT subject_id, hadm_id, charttime,specimen_id,
	...
	MAX(CASE WHEN itemid = 51000 THEN valuenum ELSE NULL END) AS triglycerides
FROM mimiciv_hosp.labevents
  WHERE itemid IN (50931,51000) AND valuenum IS NOT NULL
  GROUP BY subject_id, hadm_id, charttime, specimen_id
...
SELECT * FROM mimiciv_derived.first_day_lab f JOIN mimiciv_derived.ap_first_icu_lab ap ON f.stay_id = ap.stay_id


--DROP TABLE mimiciv_derived.test
-- SELECT * FROM mimiciv_derived.ap_first_icu_lab
-- CREATE TABLE mimiciv_derived.ap_first_icu_lab AS
-- CREATE TABLE mimiciv_derived.test AS
WITH ap AS (
...
SELECT a.subject_id, a.hadm_id, a.stay_id, a.intime, a.age, a.gender, a.race, a.los,
       dh.diabetes, dh.hypertension,
       f.wbc_max, f.rbc_max, f.rdw_max, f.hemoglobin_max, f.platelets_max,
       f.aniongap_max, f.bicarbonate_max, f.calcium_max, f.chloride_max, f.potassium_max,
       f.creatinine_max, f.glucose_max, f.triglycerides_max, f.alt_max, f.alp_max, f.ast_max, f.pt_max, f.ptt_max, f.inr_max,
	   f.abs_neutrophils_max,
       f.neutrophils_max, f.neutrophils_min, 
	   f.abs_lymphocytes_max, 
	   -- f.lymphocytes_max,
	   -- (f.abs_neutrophils_max / f.abs_lymphocytes_max) AS NLR,
	   f.albumin_max, f.albumin_min, f.bun_max, f.bun_min,
	   f.total_protein_max,
	   CASE WHEN s2.subject_id IS NOT NULL THEN 1 ELSE 0 END AS sepsis,
	   CASE WHEN s2.subject_id IS NOT NULL AND s.sofa >= 2 THEN 1 ELSE 0 END AS sepsis_3,
       f.neutrophils_max / NULLIF(f.albumin_max,0) AS NPAR,
       LN((f.triglycerides_max * f.glucose_max) / 2.0) AS TyG,
       s.sofa,
       c.has_sepsis, c.aki, c.has_chronic_kidney_disease,
       t.has_crrt, t.has_mv, t.has_ercp, t.has_vasopressin,
       -- tử vong
...
FROM ap_first_icu a
JOIN mimiciv_derived.first_day_lab f ON a.stay_id = f.stay_id 
JOIN comorbidity c ON a.subject_id = c.subject_id AND a.hadm_id = c.hadm_id AND a.stay_id = c.stay_id
JOIN treatments t  ON a.subject_id = t.subject_id AND a.hadm_id = t.hadm_id AND a.stay_id = t.stay_id
LEFT JOIN diabetes_hypertension dh ON a.subject_id = dh.subject_id AND a.hadm_id = dh.hadm_id
LEFT JOIN sofa s ON a.stay_id = s.stay_id
LEFT JOIN sepsis s2 ON a.subject_id = s2.subject_id AND a.hadm_id = s2.hadm_id
	...
	-- f.triglycerides_max IS NOT NULL;
 --  AND f.glucose_max IS NOT NULL
 --  AND f.wbc_max IS NOT NULL
 --  AND f.hemoglobin_max IS NOT NULL
 --  AND f.platelets_max IS NOT NULL
 --  AND f.alt_max IS NOT NULL
 --  AND f.ast_max IS NOT NULL
 --  AND f.albumin_max IS NOT NULL;
) SELECT * FROM rs WHERE glucose_max IS NOT NULL; 

SELECT * FROM mimiciv_derived.test WHERE  
alt_max IS NOT NULL 
AND ast_max IS NOT NULL 
AND albumin_max IS NOT NULL 
AND hemoglobin_max IS NOT NULL
AND wbc_max IS NOT NULL
AND glucose_max IS NOT NULL
AND pt_max IS NOT NULL
AND ptt_max IS NOT NULL
AND platelets_max IS NOT NULL
AND triglycerides_max IS NOT NULL;
