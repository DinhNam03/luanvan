...
WITH cbc AS (
  ...
  LEFT JOIN mimiciv_derived.complete_blood_count AS le
	...
	AND le.charttime BETWEEN ie.intime - INTERVAL '6 hours' AND ie.intime + INTERVAL '1 days'
  GROUP BY
    ie.stay_id
), chem AS (
  SELECT
    ...
    MIN(triglycerides) AS triglycerides_min,
    MAX(triglycerides) AS triglycerides_max,
...
SELECT abs_lymphocytes_max FROM mimiciv_derived.first_day_lab ORDER BY abs_lymphocytes_max
