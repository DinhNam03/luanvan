CREATE TABLE mimiciv_derived.urine_output_rate AS
WITH tm AS
(
    SELECT ie.stay_id
...
(
  ...
  , SUM(CASE WHEN EXTRACT(EPOCH FROM (io.charttime - iosum.charttime)) / 3600 <= 5
        THEN iosum.urineoutput ELSE NULL END
) AS urineoutput_6hr,

SUM(
    CASE 
        WHEN EXTRACT(EPOCH FROM (io.charttime - iosum.charttime)) / 3600 <= 5
...
SUM(
    CASE 
        WHEN EXTRACT(EPOCH FROM (io.charttime - iosum.charttime)) / 3600 <= 11
